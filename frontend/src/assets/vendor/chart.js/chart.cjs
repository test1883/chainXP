...
class Animation {
  constructor(cfg, target, prop, to) {
    ...
    this._easing =
      helpers_segment.effects[cfg.easing] || helpers_segment.effects.linear;
...
class Animations {
  ...
  configure(config) {
    ...
    Object.getOwnPropertyNames(config).forEach((key) => {
      ...
      (
        (helpers_segment.isArray(cfg.properties) && cfg.properties) || [key]
      ).forEach((prop) => {
...
function applyStack(stack, value, dsIndex, options = {}) {
  ...
  for (i = 0, ilen = keys.length; i < ilen; ++i) {
    ...
    if (
      helpers_segment.isNumberFinite(otherValue) &&
      (singleMode ||
        value === 0 ||
        helpers_segment.sign(value) === helpers_segment.sign(otherValue))
    ) {
      value += otherValue;
    }
  }
...
class DatasetController {
  ...
  linkScales() {
    ...
    const chooseId = (axis, x, y, r) =>
      axis === "x" ? x : axis === "r" ? r : y;
    const xid = (meta.xAxisID = helpers_segment.valueOrDefault(
      dataset.xAxisID,
      getFirstScaleId(chart, "x"),
    ));
    const yid = (meta.yAxisID = helpers_segment.valueOrDefault(
      dataset.yAxisID,
      getFirstScaleId(chart, "y"),
    ));
    const rid = (meta.rAxisID = helpers_segment.valueOrDefault(
      dataset.rAxisID,
  ...
  parseObjectData(meta, data, start, count) {
    ...
    for (i = 0, ilen = count; i < ilen; ++i) {
      ...
      parsed[i] = {
        x: xScale.parse(
          helpers_segment.resolveObjectKey(item, xAxisKey),
          index,
        ),
        y: yScale.parse(
          helpers_segment.resolveObjectKey(item, yAxisKey),
          index,
        ),
      };
    }
  ...
  getMinMax(scale, canStack) {
    ...
    function _skip() {
      ...
      return (
        !helpers_segment.isNumberFinite(parsed[scale.axis]) ||
        otherMin > otherValue ||
        otherMax < otherValue
      );
    }
  ...
  _update(mode) {
    ...
    meta._clip = toClip(
      helpers_segment.valueOrDefault(
        this.options.clip,
...
function getAllScaleValues(scale, type) {
  if (!scale._cache.$bar) {
    ...
    scale._cache.$bar = helpers_segment._arrayUnique(
      values.sort((a, b) => a - b),
    );
  }
...
class BarController extends DatasetController {
  ...
  parseObjectData(meta, data, start, count) {
    ...
    for (i = start, ilen = start + count; i < ilen; ++i) {
      ...
      item[iScale.axis] = iScale.parse(
        helpers_segment.resolveObjectKey(obj, iAxisKey),
        i,
      );
      parsed.push(
        parseValue(
          helpers_segment.resolveObjectKey(obj, vAxisKey),
          item,
          vScale,
          i,
        ),
  ...
  updateElements(bars, start, count, mode) {
    ...
    for (let i = start; i < start + count; i++) {
      ...
      const vpixels =
        reset || helpers_segment.isNullOrUndef(parsed[vScale.axis])
          ? {
              base,
              head: base,
            }
  ...
  _calculateBarValuePixels(index) {
    ...
    if (floating) {
      ...
      if (
        value !== 0 &&
        helpers_segment.sign(value) !== helpers_segment.sign(custom.barEnd)
      ) {
        start = 0;
      }
      start += value;
    ...
    const startValue =
      !helpers_segment.isNullOrUndef(baseValue) && !floating
        ? baseValue
        : start;
    ...
    if (base === vScale.getPixelForValue(actualBase)) {
      const halfGrid =
        (helpers_segment.sign(size) * vScale.getLineWidthForValue(actualBase)) /
        2;
      base += halfGrid;
      size -= halfGrid;
    }
  ...
  _calculateBarIndexPixels(index, ruler) {
    ...
    const maxBarThickness = helpers_segment.valueOrDefault(
...
class BubbleController extends DatasetController {
  ...
  parseArrayData(meta, data, start, count) {
    ...
    for (let i = 0; i < parsed.length; i++) {
      ...
      parsed[i]._custom = helpers_segment.valueOrDefault(
        item[2],
  ...
  resolveDataElementOptions(index, mode) {
    ...
    values.radius += helpers_segment.valueOrDefault(
...
function getRatioAndOffset(rotation, circumference, cutout) {
  ...
  if (circumference < helpers_segment.TAU) {
    ...
    const calcMax = (angle, a, b) =>
      helpers_segment._angleBetween(angle, startAngle, endAngle, true)
        ? 1
    ...
    const minY = calcMin(
      helpers_segment.PI + helpers_segment.HALF_PI,
      startY,
      endY,
    );
    ratioX = (maxX - minX) / 2;
...
class DoughnutController extends DatasetController {
  ...
  update(mode) {
    ...
    const cutout = Math.min(
      helpers_segment.toPercentage(this.options.cutout, maxSize),
      1,
    );
    ...
    const outerRadius = helpers_segment.toDimension(
      this.options.radius,
      maxRadius,
    );
  ...
  _circumference(i, reset) {
    ...
    return this.calculateCircumference(
      (meta._parsed[i] * circumference) / helpers_segment.TAU,
    );
  }
  ...
  getLabelAndValue(index) {
    ...
    const value = helpers_segment.formatNumber(
      meta._parsed[index],
...
class LineController extends DatasetController {
  ...
  update(mode) {
    ...
    let { start, count } = helpers_segment._getStartAndCountOfVisiblePoints(
      meta,
      points,
      animationsDisabled,
    );
  ...
  updateElements(points, start, count, mode) {
    ...
    const maxGapLength = helpers_segment.isNumber(spanGaps)
      ? spanGaps
...
class PolarAreaController extends DatasetController {
  ...
  parseObjectData(meta, data, start, count) {
    return helpers_segment._parseObjectDataRadialScale.bind(this)(
      meta,
      data,
      start,
      count,
...
function binarySearch(metaset, axis, value, intersect) {
  ...
  if (
    ...
    const lookupMethod = iScale._reversePixels
      ? helpers_segment._rlookupByKey
      : helpers_segment._lookupByKey;
    if (!intersect) {
...
function getIntersectItems(
  ...
  const evaluationFunc = function (element, datasetIndex, index) {
    if (
      !includeInvisible &&
      !helpers_segment._isPointInArea(element, chart.chartArea, 0)
    ) {
      return;
    }
...
function _detectPlatform(canvas) {
  if (
    !helpers_segment._isDomSupported() ||
...
function autoSkip(scale, ticks) {
  ...
  if (numMajorIndices > 0) {
    ...
    skip(
      ...
      newTicks,
      spacing,
      helpers_segment.isNullOrUndef(avgMajorSpacing)
        ? 0
...
function skip(ticks, newTicks, spacing, majorStart, majorEnd) {
  ...
  const end = Math.min(
    helpers_segment.valueOrDefault(majorEnd, ticks.length),
    ticks.length,
  );
  let count = 0;
  let length, i, next;
...
function titleArgs(scale, offset, position, align) {
  ...
  if (scale.isHorizontal()) {
    ...
    rotation =
      position === "left" ? -helpers_segment.HALF_PI : helpers_segment.HALF_PI;
  }
  return {
    titleX,
    titleY,
...
class Scale extends Element {
  ...
  getUserBounds() {
    ...
    _userMin = helpers_segment.finiteOrDefault(
      _userMin,
      Number.POSITIVE_INFINITY,
    );
    _userMax = helpers_segment.finiteOrDefault(
      _userMax,
      Number.NEGATIVE_INFINITY,
    );
    _suggestedMin = helpers_segment.finiteOrDefault(
      _suggestedMin,
      Number.POSITIVE_INFINITY,
    );
    _suggestedMax = helpers_segment.finiteOrDefault(
      _suggestedMax,
      Number.NEGATIVE_INFINITY,
    );
    return {
  ...
  getMinMax(canStack) {
    ...
    return {
      min: helpers_segment.finiteOrDefault(
        min,
        helpers_segment.finiteOrDefault(max, min),
      ),
      max: helpers_segment.finiteOrDefault(
        max,
        helpers_segment.finiteOrDefault(min, max),
      ),
    };
  }
  getPadding() {
  ...
  generateTickLabels(ticks) {
    ...
    for (i = 0, ilen = ticks.length; i < ilen; i++) {
      tick = ticks[i];
      tick.label = helpers_segment.callback(
        tickOpts.callback,
        [tick.value, i, ticks],
        this,
      );
  ...
  calculateLabelRotation() {
    ...
    const maxWidth = helpers_segment._limitValue(
    ...
    if (maxLabelWidth + 6 > tickWidth) {
      ...
      labelRotation = helpers_segment.toDegrees(
        Math.min(
          Math.asin(
            helpers_segment._limitValue(
          ...
          Math.asin(
            helpers_segment._limitValue(maxHeight / maxLabelDiagonal, -1, 1),
          ) -
            Math.asin(
              helpers_segment._limitValue(
  ...
  _computeLabelSizes(ticks, length, maxTicksLimit) {
    ...
    for (i = 0; i < length; i += increment) {
      ...
      width = height = 0;
      if (
        !helpers_segment.isNullOrUndef(label) &&
        !helpers_segment.isArray(label)
      ) {
        width = helpers_segment._measureText(
          ctx,
          cache.data,
          cache.gc,
          width,
        ...
        for (j = 0, jlen = label.length; j < jlen; ++j) {
          nestedLabel = label[j];
          if (
            !helpers_segment.isNullOrUndef(nestedLabel) &&
            !helpers_segment.isArray(nestedLabel)
          ) {
            width = helpers_segment._measureText(
              ctx,
              cache.data,
  ...
  getPixelForDecimal(decimal) {
    ...
    return helpers_segment._int16Range(
      this._alignToPixels
        ? helpers_segment._alignPixel(this.chart, pixel, 0)
        : pixel,
    );
  }
  ...
  _computeGridLineItems(chartArea) {
    ...
    const limit = helpers_segment.valueOrDefault(
      options.ticks.maxTicksLimit,
      ticksLength,
    );
    ...
    for (i = 0; i < ticksLength; i += step) {
      ...
      alignedLineValue = helpers_segment._alignPixel(
        chart,
        lineValue,
        lineWidth,
      );
  ...
  _computeLabelItems(chartArea) {
    ...
    for (i = 0, ilen = ticks.length; i < ilen; ++i) {
      ...
      if (optsAtIndex.showLabelBackdrop) {
        const labelPadding = helpers_segment.toPadding(
  ...
  drawBorder() {
    ...
    if (this.isHorizontal()) {
      x1 =
        helpers_segment._alignPixel(chart, this.left, axisWidth) -
        axisWidth / 2;
      x2 =
        helpers_segment._alignPixel(chart, this.right, lastLineWidth) +
        lastLineWidth / 2;
      y1 = y2 = borderValue;
    } else {
      y1 =
        helpers_segment._alignPixel(chart, this.top, axisWidth) - axisWidth / 2;
      y2 =
        helpers_segment._alignPixel(chart, this.bottom, lastLineWidth) +
        lastLineWidth / 2;
      x1 = x2 = borderValue;
    }
    ctx.save();
  ...
  drawTitle() {
    ...
    if (
      position === "bottom" ||
      position === "center" ||
      helpers_segment.isObject(position)
    ) {
...
function routeDefaults(scope, routes) {
  Object.keys(routes).forEach((property) => {
    ...
    helpers_segment.defaults.route(
      sourceScope,
      sourceName,
      targetScope,
      targetName,
...
class PluginService {
  ...
  _notify(descriptors, chart, hook, args) {
    ...
    for (const descriptor of descriptors) {
      ...
      if (
        helpers_segment.callback(method, params, plugin) === false &&
        args.cancelable
      ) {
        return false;
      }
  ...
  _createDescriptors(chart, all) {
    ...
    const options = helpers_segment.valueOrDefault(
...
function mergeScaleConfig(config, options) {
  ...
  Object.keys(configScales).forEach((id) => {
    ...
    const axis = determineAxis(
      ...
      helpers_segment.defaults.scales[scaleConf.type],
    );
...
class Config {
  ...
  getOptionScopes(mainScope, keyLists, resetCache) {
    ...
    keyLists.forEach((keys) => {
      ...
      keys.forEach((key) =>
        addIfFound(scopes, helpers_segment.overrides[type] || {}, key),
      );
      keys.forEach((key) => addIfFound(scopes, helpers_segment.defaults, key));
      keys.forEach((key) =>
        addIfFound(scopes, helpers_segment.descriptors, key),
      );
    });
  ...
  createResolver(scopes, context, prefixes = [""], descriptorDefaults) {
    ...
    return helpers_segment.isObject(context)
      ? helpers_segment._attachContext(
          resolver,
          context,
          undefined,
          descriptorDefaults,
...
const hasFunction = (value) =>
  helpers_segment.isObject(value) &&
  Object.getOwnPropertyNames(value).reduce(
    (acc, key) => acc || helpers_segment.isFunction(value[key]),
    false,
  );
function needContext(proxy, names) {
  ...
  for (const prop of names) {
    ...
    if (
      (scriptable &&
        (helpers_segment.isFunction(value) || hasFunction(value))) ||
      (indexable && helpers_segment.isArray(value))
    ) {
      return true;
    }
  }
...
class Chart {
  ...
  constructor(item, userConfig) {
    ...
    this.$context = undefined;
    this._doResize = helpers_segment.debounce(
  ...
  buildOrUpdateScales() {
    ...
    helpers_segment.each(items, (item) => {
      ...
      const axis = determineAxis(id, scaleOptions);
      const scaleType = helpers_segment.valueOrDefault(
        scaleOptions.type,
        item.dtype,
      );
      if (
  ...
  _checkEventBindings() {
    ...
    if (
      !helpers_segment.setsEqual(existingEvents, newEvents) ||
  ...
  _updateDatasets(mode) {
    ...
    for (let i = 0, ilen = this.data.datasets.length; i < ilen; ++i) {
      this._updateDataset(
        i,
        helpers_segment.isFunction(mode)
          ? mode({
              datasetIndex: i,
            })
          : mode,
  ...
  isPointInArea(point) {
    return helpers_segment._isPointInArea(
      point,
      this.chartArea,
      this._minPadding,
    );
  ...
  getContext() {
    return (
      this.$context ||
      (this.$context = helpers_segment.createContext(null, {
        chart: this,
        type: "chart",
      }))
    );
...
function invalidatePlugins() {
  return helpers_segment.each(Chart.instances, (chart) =>
    chart._plugins.invalidate(),
  );
}
...
function clipArc(ctx, element, endAngle) {
  ...
  if (innerRadius > pixelMargin) {
    ...
    ctx.arc(
      ...
      y,
      pixelMargin,
      endAngle + helpers_segment.HALF_PI,
      startAngle - helpers_segment.HALF_PI,
    );
  }
  ctx.closePath();
  ctx.clip();
 ...
 */ function parseBorderRadius$1(arc, innerRadius, outerRadius, angleDelta) {
  ...
  const computeOuterLimit = (val) => {
    ...
    return helpers_segment._limitValue(
      val,
      0,
 ...
 */ function pathArc(ctx, element, offset, spacing, end, circular) {
  ...
  const beta =
    Math.max(0.001, alpha * outerRadius - offset / helpers_segment.PI) /
    outerRadius;
...
function drawArc(ctx, element, offset, spacing, circular) {
  ...
  if (fullCircles) {
    ...
    if (!isNaN(circumference)) {
      endAngle =
        startAngle +
        (circumference % helpers_segment.TAU || helpers_segment.TAU);
    }
  }
  pathArc(ctx, element, offset, spacing, endAngle, circular);
  ctx.fill();
...
class ArcElement extends Element {
  ...
  inRange(chartX, chartY, useFinalPosition) {
    ...
    const _circumference = helpers_segment.valueOrDefault(
      circumference,
      endAngle - startAngle,
    );
    const betweenAngles =
      _circumference >= helpers_segment.TAU ||
      helpers_segment._angleBetween(angle, startAngle, endAngle);
    const withinRadius = helpers_segment._isBetween(
      distance,
      innerRadius + rAdjust,
      outerRadius + rAdjust,
    );
  ...
  draw(ctx) {
    ...
    this.fullCircles =
      circumference > helpers_segment.TAU
        ? Math.floor(circumference / helpers_segment.TAU)
        : 0;
...
function setStyle(ctx, options, style = options) {
  ctx.lineCap = helpers_segment.valueOrDefault(
    style.borderCapStyle,
    options.borderCapStyle,
  );
  ctx.setLineDash(
    helpers_segment.valueOrDefault(style.borderDash, options.borderDash),
  );
  ctx.lineDashOffset = helpers_segment.valueOrDefault(
    style.borderDashOffset,
    options.borderDashOffset,
  );
  ctx.lineJoin = helpers_segment.valueOrDefault(
    style.borderJoinStyle,
    options.borderJoinStyle,
  );
  ctx.lineWidth = helpers_segment.valueOrDefault(
    style.borderWidth,
    options.borderWidth,
  );
  ctx.strokeStyle = helpers_segment.valueOrDefault(
    style.borderColor,
    options.borderColor,
  );
}
...
class LineElement extends Element {
  ...
  updateControlPoints(chartArea, indexAxis) {
    ...
    if (
      ...
      helpers_segment._updateBezierControlPoints(
        this._points,
        options,
        chartArea,
        loop,
  ...
  get segments() {
    return (
      this._segments ||
      (this._segments = helpers_segment._computeSegments(
        this,
...
class PointElement extends Element {
  ...
  draw(ctx, area) {
    ...
    if (
      this.skip ||
      options.radius < 0.1 ||
      !helpers_segment._isPointInArea(this, area, this.size(options) / 2)
    ) {
      return;
    }
    ctx.strokeStyle = options.borderColor;
...
function inRange(bar, x, y, useFinalPosition) {
  ...
  return (
    bounds &&
    (skipX || helpers_segment._isBetween(x, bounds.left, bounds.right)) &&
    (skipY || helpers_segment._isBetween(y, bounds.top, bounds.bottom))
  );
}
...
class BarElement extends Element {
  ...
  draw(ctx) {
    ...
    const addRectPath = hasRadius(outer.radius)
      ? helpers_segment.addRoundedRectPath
      : addNormalRectPath;
    ctx.save();
...
function minMaxDecimation(data, start, count, availableWidth) {
  ...
  for (i = start; i < start + count; ++i) {
    ...
    if (truncX === prevX) {
      ...
      const lastIndex = i - 1;
      if (
        !helpers_segment.isNullOrUndef(minIndex) &&
        !helpers_segment.isNullOrUndef(maxIndex)
      ) {
...
function getStartAndCountOfVisiblePointsSimplified(meta, points) {
  ...
  if (minDefined) {
    start = helpers_segment._limitValue(
      helpers_segment._lookupByKey(points, iScale.axis, min).lo,
      0,
      pointCount - 1,
    );
  }
  if (maxDefined) {
    count =
      helpers_segment._limitValue(
        helpers_segment._lookupByKey(points, iScale.axis, max).hi + 1,
        start,
        pointCount,
      ) - start;
  } else {
...
var plugin_decimation = {
  ...
  beforeElementsUpdate: (chart, args, options) => {
    ...
    chart.data.datasets.forEach((dataset, datasetIndex) => {
      ...
      if (
        helpers_segment.resolve([indexAxis, chart.options.indexAxis]) === "y"
      ) {
        return;
      }
...
function _segments(line, target, property) {
  ...
  for (const segment of segments) {
    ...
    for (const tgt of targetSegments) {
      ...
      const fillSources = helpers_segment._boundSegment(
        segment,
        points,
        subBounds,
      );
...
function parseFillOption(line) {
  ...
  let fill = helpers_segment.valueOrDefault(
...
class Legend extends Element {
  ...
  buildLabels() {
    ...
    let legendItems =
      helpers_segment.callback(labelOpts.generateLabels, [this.chart], this) ||
      [];
    if (labelOpts.filter) {
  ...
  adjustHitBoxes() {
    ...
    if (this.isHorizontal()) {
      let row = 0;
      let left = helpers_segment._alignStartEnd(
        align,
        this.left + padding,
      ...
      for (const hitbox of hitboxes) {
        if (row !== hitbox.row) {
          row = hitbox.row;
          left = helpers_segment._alignStartEnd(
            align,
            this.left + padding,
            this.right - this.lineWidths[row],
          );
      ...
      let col = 0;
      let top = helpers_segment._alignStartEnd(
        align,
      ...
      for (const hitbox of hitboxes) {
        if (hitbox.col !== col) {
          col = hitbox.col;
          top = helpers_segment._alignStartEnd(
            align,
            this.top + titleHeight + padding,
            this.bottom - this.columnSizes[col].height,
          );
  ...
  _draw() {
    ...
    const rtlHelper = helpers_segment.getRtlAdapter(
      opts.rtl,
      this.left,
      this.width,
    );
    ...
    const drawLegendBox = function (x, y, legendItem) {
      ...
      ctx.fillStyle = helpers_segment.valueOrDefault(
      ...
      if (labelOpts.usePointStyle) {
        ...
        helpers_segment.drawPointLegend(
          ctx,
          drawOptions,
          centerX,
          centerY,
        ...
        const borderRadius = helpers_segment.toTRBLCorners(
    ...
    if (isHorizontal) {
      cursor = {
        x: helpers_segment._alignStartEnd(
          align,
          this.left + padding,
      ...
      cursor = {
        x: this.left + padding,
        y: helpers_segment._alignStartEnd(
          align,
          this.top + titleHeight + padding,
    ...
    this.legendItems.forEach((legendItem, i) => {
      ...
      if (isHorizontal) {
        if (i > 0 && x + width + padding > this.right) {
          ...
          cursor.line++;
          x = cursor.x = helpers_segment._alignStartEnd(
            align,
            this.left + padding,
        ...
        cursor.line++;
        y = cursor.y = helpers_segment._alignStartEnd(
          align,
          this.top + titleHeight + padding,
      ...
      if (isHorizontal) {
        ...
        cursor.y +=
          calculateLegendItemHeight(legendItem, fontLineHeight) + padding;
      } else {
        cursor.y += lineHeight;
      }
    });
  ...
  drawTitle() {
    ...
    if (this.isHorizontal()) {
      ...
      y =
        ...
        helpers_segment._alignStartEnd(
          opts.align,
          this.top,
          this.bottom -
            maxHeight -
    ...
    ctx.textAlign = rtlHelper.textAlign(
      helpers_segment._toLeftRightCenter(position),
    );
    ctx.textBaseline = "middle";
  ...
  _getLegendItemAt(x, y) {
    let i, hitBox, lh;
    if (
      helpers_segment._isBetween(x, this.left, this.right) &&
      helpers_segment._isBetween(y, this.top, this.bottom)
    ) {
      lh = this.legendHitBoxes;
      for (i = 0; i < lh.length; ++i) {
        hitBox = lh[i];
        if (
          helpers_segment._isBetween(
            x,
            hitBox.left,
            hitBox.left + hitBox.width,
          ) &&
          helpers_segment._isBetween(y, hitBox.top, hitBox.top + hitBox.height)
        ) {
...
class Title extends Element {
  ...
  update(maxWidth, maxHeight) {
    ...
    const textSize =
      lineCount * helpers_segment.toFont(opts.font).lineHeight +
...
class Tooltip extends Element {
  ...
  _drawColorBox(ctx, pt, i, rtlHelper, options) {
    ...
    if (options.usePointStyle) {
      ...
      ctx.lineWidth = helpers_segment.isObject(labelColor.borderWidth)
  ...
  handleEvent(e, replay, inChartArea = true) {
    ...
    const changed =
      replay ||
      !helpers_segment._elementsEqual(active, lastActive) ||
      positionChanged;
    if (changed) {
      this._active = active;
...
const validIndex = (index, max) =>
  index === null
    ? null
    : helpers_segment._limitValue(Math.round(index), 0, max);
    ...
    index =
      isFinite(index) && labels[index] === raw
        ...
        : findOrAddLabel(
            labels,
            raw,
            helpers_segment.valueOrDefault(index, raw),
            this._addedLabels,
          );
...
function generateTicks$1(generationOptions, dataRange) {
  ...
  let spacing =
    helpers_segment.niceNum((rmax - rmin) / maxSpaces / unit) * unit;
  ...
  if (numSpaces > maxSpaces) {
    spacing =
      helpers_segment.niceNum((numSpaces * spacing) / maxSpaces / unit) * unit;
  }
  ...
  if (
    ...
    maxDefined &&
    step &&
    helpers_segment.almostWhole((max - min) / step, spacing / 1000)
  ) {
  ...
  const decimalPlaces = Math.max(
    helpers_segment._decimalPlaces(spacing),
    helpers_segment._decimalPlaces(niceMin),
  );
  factor = Math.pow(
    10,
    helpers_segment.isNullOrUndef(precision) ? decimalPlaces : precision,
  );
...
class LinearScaleBase extends Scale {
  ...
  getLabelForValue(value) {
    return helpers_segment.formatNumber(
      value,
...
class LinearScale extends LinearScaleBase {
  ...
  computeTickLimit() {
    ...
    const minRotation = helpers_segment.toRadians(
...
function generateTicks(generationOptions, { min, max }) {
  ...
  let value = helpers_segment.finiteOrDefault(
  ...
  const lastTick = helpers_segment.finiteOrDefault(
...
class LogarithmicScale extends Scale {
  ...
  determineDataLimits() {
    ...
    if (
      ...
      !helpers_segment.isNumberFinite(this._userMin)
    ) {
      this.min =
  ...
  configure() {
    ...
    this._valueRange =
      helpers_segment.log10(this.max) - helpers_segment.log10(start);
  }
  getPixelForValue(value) {
    ...
    return this.getPixelForDecimal(
      value === this.min
        ? 0
        : (helpers_segment.log10(value) - this._startValue) / this._valueRange,
    );
  }
  getValueForPixel(pixel) {
  ...
  if (tickOpts.display && opts.display) {
    ...
    return (
      helpers_segment.valueOrDefault(
        ...
        helpers_segment.defaults.font.size,
      ) + padding.height
    );
  }
  return 0;
...
function fitWithPointLabels(scale) {
  ...
  for (let i = 0; i < valueCount; i++) {
    ...
    const angleRadians = helpers_segment._normalizeAngle(
...
function createPointLabelItem(scale, index, itemOpts) {
  ...
  const angle = Math.round(
    helpers_segment.toDegrees(
      helpers_segment._normalizeAngle(
        pointLabelPosition.angle + helpers_segment.HALF_PI,
      ),
    ),
  );
...
function isNotOverlapped(item, area) {
  ...
  const apexesInArea =
    helpers_segment._isPointInArea(
      {
        x: left,
        y: top,
      },
...
class RadialLinearScale extends LinearScaleBase {
  ...
  setDimensions() {
    const padding = (this._padding = helpers_segment.toPadding(
  ...
  generateTickLabels(ticks) {
    ...
    this._pointLabels = this.getLabels()
      .map((value, index) => {
        const label = helpers_segment.callback(
  ...
  getIndexAngle(index) {
    const angleMultiplier =
      helpers_segment.TAU / (this._pointLabels.length || 1);
    const startAngle = this.options.startAngle || 0;
    return helpers_segment._normalizeAngle(
      index * angleMultiplier + helpers_segment.toRadians(startAngle),
    );
  }
  ...
  getPointPosition(index, distanceFromCenter, additionalAngle = 0) {
    const angle =
      this.getIndexAngle(index) - helpers_segment.HALF_PI + additionalAngle;
    return {
...
function parse(scale, input) {
  ...
  if (round) {
    value =
      round === "week" &&
      (helpers_segment.isNumber(isoWeekday) || isoWeekday === true)
...
class TimeScale extends Scale {
  ...
  determineDataLimits() {
    ...
    min =
      helpers_segment.isNumberFinite(min) && !isNaN(min)
        ? min
        : +adapter.startOf(Date.now(), unit);
    max =
      helpers_segment.isNumberFinite(max) && !isNaN(max)
        ? max
  ...
  _getLabelSize(label) {
    ...
    const angle = helpers_segment.toRadians(
